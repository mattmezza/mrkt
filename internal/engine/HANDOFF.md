...
Event keys retain canonical request fingerprints independently of retained payloads: exact replays return the original event, changed type/contact/payload/explicit occurrence time conflicts, and legacy keys without reconstructable fingerprints fail closed. Contact events enroll only their contact; project events create broadcasts only. Enrollment entry keys enforce `once` versus per-event reentry while retaining the actual event ID for `Event.*` rendering.

Broadcast pause/resume/cancel transitions are audited. Paused work remains retryable, cancelled work is cancelled, and the final delivery-intent transaction requires a queued broadcast or waiting enrollment. Every sequence completion path uses one transactional helper guarded by `completion_emitted`, producing exactly one `sequence.completed` outbox event.
